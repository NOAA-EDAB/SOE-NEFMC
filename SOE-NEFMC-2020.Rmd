...
```{r gb-heatwave-max, fig.cap="Maximum intensity heatwave anomaly in Georges Bank occurring on August 23, 2019."}
#EPU shapefile
ne_epu_sf <- ecodata::epu_sf %>% 
  filter(EPU %in% c("GOM","GB"))
...
```{r gb-heatwave, eval= T, echo = F, fig.width = 8, fig.asp = .35, fig.cap="Marine heatwave cumulative intensity (left) and maximum intensity (right) in the Gulf of Maine ."}
# cumu <- ecodata::heatwave %>% 
#   filter(Var == "cumulative intensity") %>% 
#   mutate(Var = recode(Var, "cumulative intensity" = "Cumulative Intensity (degree C x days)"))
# 
...
Fluctuations in primary production over time (Fig. \ref{fig:NE-sli})  may relate to observed patterns in copeopod size structure (Fig. \ref{fig:NE-zoo-abund}). This period also corresponds with regime shifts in fish recruitment [@perretti_regime_2017].
