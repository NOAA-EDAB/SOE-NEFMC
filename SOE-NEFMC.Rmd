...
NE_com_plot <- ggplot(dat_com_e_NE, aes(x=ComEng, y=ComRel)) + 
  geom_point(shape=1, 
  ...
  ylab(expression(paste("Population Relative Commercial Engagement Index", italic(" (factor score)"))))+
...
NE_com_poverty <- ggplot(dat_com_e_NE %>% filter(poverty_rank == "high"|
...
NE_com_pers <- ggplot(dat_com_e_NE %>% filter(personal_disruption_rank == "high"|personal_disruption_rank == "med high"|personal_disruption_rank =="med"), aes(x=ComEng, y=ComRel)) + 
  geom_point(shape=1, 
             size=1.4, stroke=0.9, alpha=0.6, 
...
NE_com_pop <- ggplot(dat_com_e_NE %>% filter(pop_composition_rank == "high"| 
...
#put them together
#tiff("NE_com_1-23-25.tiff", units="in", width=8, height=5, res=300)

NE_com_plot + (NE_com_poverty / NE_com_pers / NE_com_pop) +
...
NE_rec_plot <- ggplot(dat_rec_e_NE, aes(x=RecEng, y=RecRel)) + 
  geom_point(shape=1, 
             size=1.4, stroke=0.9, alpha=0.6, 
             aes(color=RecEng_ct))+
  scale_color_manual(values = c("high" = "navy",
...
NE_rec_poverty <- ggplot(dat_rec_e_NE %>% filter(poverty_rank == "high"|
                                                   poverty_rank == "med high"|
...
NE_rec_pers <- ggplot(dat_rec_e_NE %>% filter(personal_disruption_rank == "high"|
...
NE_rec_pop <- ggplot(dat_rec_e_NE %>% filter(pop_composition_rank == "high"| 
                                               pop_composition_rank == "med high"|
                                               pop_composition_rank =="med"), 
                     aes(x=RecEng, y=RecRel)) + 
  geom_point(shape=1, 
...
#put them together
#tiff("NE_rec_1-23-25.tiff", units="in", width=8, height=5, res=300)

NE_rec_plot + (NE_rec_poverty / NE_rec_pers / NE_rec_pop) +
  plot_layout(ncol=2, widths = c(5,2))
...
```{r commvulex, fig.width = 7.5, fig.asp=.5, fig.cap="Proportion of New England communities at each revenue climate vulnerability level over time."}
...
```
ADD COG FOR BENTHOS AND ZOOPLANKTON?

In contrast, macrobenthos center of gravity has shifted westward. Macrobenthos are small bottom-dwelling invertebrates including polychaete worms, small crustaceans, bivalves (non-commercial), gastropods, nemerteans, tunicates, cnidarians, brittle stars, sea cucumbers, and sand dollars. Large copepods have a similar pattern to macrobenthos (link to catalog).
...
While small Copepods center of gravity from ECOMON shows generally similar shifts to forage fish, [link to catalog dont need to show this].
```{r Smcopeshifts, fig.alt="Eastward (left) and northward (right) shifts in the center of gravity for small copepod species on the Northeast U.S. Shelf", fig.width = 8,fig.asp = .3}
#, fig.asp = .3
...
```{r GSI, fig.cap = "Index representing changes in the location of the western Gulf Stream north wall. Positive values represent a more northerly Gulf Stream position.", fig.width = 7.5, fig.asp = .3}
